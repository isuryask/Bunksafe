...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Plus Jakarta Sans', -apple-system, BlinkMacSystemFont, sans-serif; -webkit-tap-highlight-color: transparent; }
  ...
  .app-logo {
    width: 28px; height: 28px; background: linear-gradient(135deg, #0284c7 0%, #38bdf8 100%);
    border-radius: var(--radius-sm); display: flex; align-items: center; justify-content: center;
    font-size: 0.85rem; font-weight: 800; color: #fff; box-shadow: 0 2px 10px rgba(2, 132, 199, 0.3);
  }
  .app-title { font-size: 0.95rem; font-weight: 700; letter-spacing: -0.02em; color: var(--text-high); }
  .sys-badge {
    font-family: 'JetBrains Mono', monospace; font-size: 0.68rem; font-weight: 600;
    padding: 3px 8px; border-radius: 999px; background: var(--surface-subtle);
    border: 1px solid var(--border); color: var(--text-low);
  }

  .panel {
    background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius-lg);
    padding: 18px; margin-bottom: 14px; box-shadow: 0 4px 20px -2px rgba(0, 0, 0, 0.4); transition: var(--transition);
  }
  ...
  .hero-panel {
    ...
    padding: 24px 20px; margin-bottom: 18px; text-align: center; box-shadow: 0 12px 36px -8px rgba(0, 0, 0, 0.6);
  }
  .hero-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; }
  .hero-label { font-size: 0.72rem; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; color: var(--text-med); }
  .hero-status-tag { display: inline-flex; align-items: center; gap: 5px; padding: 4px 10px; border-radius: 999px; font-size: 0.72rem; font-weight: 700; }
  ...
  .hero-metric-wrap { display: flex; align-items: center; justify-content: center; gap: 24px; margin: 12px 0 16px; }
  .radial-holder { position: relative; width: 100px; height: 100px; }
  .radial-svg { transform: rotate(-90deg); width: 100px; height: 100px; }
  .radial-bg { stroke: rgba(255, 255, 255, 0.06); stroke-width: 8; fill: transparent; }
  .radial-meter { stroke-width: 8; stroke-linecap: round; fill: transparent; stroke-dasharray: 264; stroke-dashoffset: 264; transition: stroke-dashoffset 0.8s cubic-bezier(0.16, 1, 0.3, 1), stroke 0.3s; }
  .radial-center-text { position: absolute; inset: 0; display: flex; align-items: center; justify-content: center; font-family: 'JetBrains Mono', monospace; font-size: 1.25rem; font-weight: 800; }
  
  .hero-stats-grid { text-align: left; }
  .stat-unit-label { font-size: 0.68rem; font-weight: 600; color: var(--text-low); text-transform: uppercase; letter-spacing: 0.05em; }
  .stat-unit-val { font-family: 'JetBrains Mono', monospace; font-size: 1.15rem; font-weight: 700; color: var(--text-high); margin-bottom: 6px; }

  .hero-footer-meta {
    padding-top: 12px; border-top: 1px solid var(--border); font-size: 0.78rem;
    color: var(--text-med); display: flex; justify-content: space-between; align-items: center;
  }

  .control-label {
    display: flex; justify-content: space-between; align-items: center;
    font-size: 0.75rem; font-weight: 700; color: var(--text-med); margin-bottom: 8px; text-transform: uppercase; letter-spacing: 0.05em;
  }
  .control-label span.hint { font-size: 0.68rem; color: var(--text-low); text-transform: none; font-weight: 500; }

  select, input[type="date"], input[type="text"], input[type="number"] {
    width: 100%; background: var(--bg); border: 1px solid var(--border); color: var(--text-high);
    padding: 11px 14px; border-radius: var(--radius-md); font-size: 0.88rem; font-weight: 500;
    margin-bottom: 12px; outline: none; transition: var(--transition);
  }
  select:focus, input:focus { border-color: var(--primary); box-shadow: 0 0 0 3px var(--primary-dim); }

  .grid-2 { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }

  .dropzone {
    position: relative; border: 1.5px dashed var(--border-hover); background: var(--surface-subtle);
    border-radius: var(--radius-md); padding: 18px 14px; text-align: center; cursor: pointer; transition: var(--transition); margin-bottom: 14px;
  }
  .dropzone:hover { border-color: var(--primary); background: var(--primary-dim); }
  .dropzone.ready { border-style: solid; border-color: var(--safe-border); background: var(--safe-dim); }
  .dropzone input[type="file"] { position: absolute; inset: 0; opacity: 0; width: 100%; height: 100%; cursor: pointer; z-index: 5; }
  .dropzone-state-empty { display: flex; flex-direction: column; align-items: center; gap: 4px; }
  .dz-icon { font-size: 1.3rem; margin-bottom: 2px; }
  .dz-title { font-size: 0.84rem; font-weight: 700; color: var(--text-high); }
  .dz-sub { font-size: 0.72rem; color: var(--text-low); }
  .dropzone-state-loaded { display: none; align-items: center; gap: 12px; text-align: left; }
  .dz-thumb { width: 42px; height: 42px; border-radius: var(--radius-sm); object-fit: cover; border: 1px solid var(--border); }
  .dz-meta { flex: 1; min-width: 0; }
  .dz-filename { font-size: 0.82rem; font-weight: 700; color: var(--text-high); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  .dz-tag { font-size: 0.7rem; font-weight: 700; color: var(--safe); display: flex; align-items: center; gap: 4px; margin-top: 2px; }

  .disclosure-panel { background: var(--surface-elevated); border: 1px solid var(--border); border-radius: var(--radius-md); margin-bottom: 10px; overflow: hidden; }
  .disclosure-header { padding: 12px 14px; display: flex; justify-content: space-between; align-items: center; cursor: pointer; user-select: none; font-size: 0.8rem; font-weight: 700; color: var(--text-high); transition: var(--transition); }
  .disclosure-header:hover { background: var(--surface-subtle); }
  .disclosure-counter { font-family: 'JetBrains Mono', monospace; font-size: 0.68rem; background: var(--surface); border: 1px solid var(--border); padding: 2px 7px; border-radius: 999px; color: var(--text-med); margin-left: 8px; }
  .disclosure-arrow { font-size: 0.7rem; color: var(--text-low); transition: transform 0.2s ease; }
  .disclosure-panel.open .disclosure-arrow { transform: rotate(180deg); }
  .disclosure-body { display: none; padding: 12px 14px 14px; border-top: 1px solid var(--border); }
  .disclosure-panel.open .disclosure-body { display: block; }

  .seg-control { display: flex; background: var(--bg); padding: 3px; border-radius: var(--radius-sm); border: 1px solid var(--border); margin-bottom: 10px; }
  .seg-btn { flex: 1; background: transparent; border: none; color: var(--text-med); padding: 6px 4px; font-size: 0.72rem; font-weight: 600; border-radius: 6px; cursor: pointer; transition: var(--transition); }
  .seg-btn.active { background: var(--surface-elevated); color: var(--text-high); box-shadow: 0 2px 8px rgba(0, 0, 0, 0.4); }

  .inline-row { display: flex; gap: 8px; align-items: center; }
  .inline-row input, .inline-row select { margin-bottom: 0; flex: 1; }
  .action-btn { padding: 10px 14px; background: var(--surface-elevated); border: 1px solid var(--border); border-radius: var(--radius-md); color: var(--text-high); font-size: 0.78rem; font-weight: 700; cursor: pointer; white-space: nowrap; transition: var(--transition); }
  .action-btn:active { transform: scale(0.97); }

  .tags-shelf { display: flex; flex-wrap: wrap; gap: 6px; min-height: 28px; margin-top: 10px; }
  .tag-item { font-family: 'JetBrains Mono', monospace; font-size: 0.7rem; font-weight: 600; padding: 3px 8px; border-radius: var(--radius-sm); display: inline-flex; align-items: center; gap: 6px; }
  .tag-item span { cursor: pointer; opacity: 0.6; }
  ...
  .cal-matrix { background: var(--bg); border: 1px solid var(--border); border-radius: var(--radius-md); padding: 10px; margin-top: 10px; }
  .cal-matrix-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; }
  .cal-matrix-title { font-size: 0.8rem; font-weight: 700; }
  .cal-nav-btn { background: var(--surface-elevated); border: 1px solid var(--border); color: var(--text-high); padding: 3px 8px; border-radius: var(--radius-sm); font-size: 0.72rem; cursor: pointer; }
  .cal-grid-row { display: grid; grid-template-columns: repeat(7, 1fr); gap: 3px; text-align: center; }
  .cal-col-lbl { font-size: 0.62rem; font-weight: 700; color: var(--text-low); padding: 2px 0; }
  .cal-cell-day { aspect-ratio: 1; display: flex; align-items: center; justify-content: center; font-size: 0.72rem; font-family: 'JetBrains Mono', monospace; font-weight: 600; border-radius: 6px; cursor: pointer; border: 1px solid transparent; color: var(--text-high); background: rgba(255, 255, 255, 0.02); }
  .cal-cell-day.empty { background: transparent; cursor: default; }
  .cal-cell-day.is-holiday { background: var(--holiday-dim); border-color: rgba(168, 85, 247, 0.5); color: #d8b4fe; font-weight: 800; }
  .cal-cell-day.is-sun { color: var(--text-low); opacity: 0.4; }

  .simulator-card { display: none; margin-bottom: 16px; }
  .sim-metric-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; }
  .sim-val-display { font-family: 'JetBrains Mono', monospace; font-size: 0.88rem; font-weight: 700; color: var(--primary); }
  input[type="range"] { width: 100%; accent-color: var(--primary); cursor: pointer; background: transparent; margin-bottom: 0; }

  .subject-card { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius-lg); padding: 16px; margin-bottom: 12px; box-shadow: 0 4px 16px rgba(0, 0, 0, 0.3); transition: var(--transition); }
  ...
  .subject-name { font-size: 0.92rem; font-weight: 700; color: var(--text-high); line-height: 1.3; }
  .subject-pct-pill { font-family: 'JetBrains Mono', monospace; font-size: 0.74rem; font-weight: 800; padding: 3px 8px; border-radius: 6px; white-space: nowrap; }

  .gauge-track { position: relative; height: 7px; background: var(--bg); border-radius: 999px; margin-bottom: 12px; border: 1px solid rgba(255, 255, 255, 0.04); }
  .gauge-fill { height: 100%; border-radius: 999px; transition: width 0.3s ease; }
  .gauge-pin-80 { position: absolute; top: -3px; bottom: -3px; left: 80%; width: 2px; background: #fff; box-shadow: 0 0 6px rgba(255, 255, 255, 0.8); z-index: 2; }
  .gauge-pin-text { position: absolute; right: calc(20% - 10px); top: -15px; font-size: 0.58rem; font-family: 'JetBrains Mono', monospace; font-weight: 800; color: var(--text-low); }

  .subject-detail-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 6px; font-size: 0.78rem; color: var(--text-med); }
  .subject-detail-grid b { color: var(--text-high); font-family: 'JetBrains Mono', monospace; font-size: 0.78rem; }
  
  .insight-box { grid-column: span 2; padding: 7px 10px; border-radius: var(--radius-sm); font-size: 0.76rem; font-weight: 600; margin-top: 4px; }
  ...
  .proj-strip { grid-column: span 2; display: flex; justify-content: space-between; align-items: center; color: var(--primary); font-size: 0.76rem; font-weight: 600; margin-top: 2px; font-family: 'JetBrains Mono', monospace; }

  .mobile-float-bar {
    position: fixed; bottom: 0; left: 0; right: 0; background: rgba(9, 10, 15, 0.85);
    backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px); border-top: 1px solid var(--border);
    padding: 12px 16px calc(12px + env(safe-area-inset-bottom)); z-index: 50;
  }
  .float-inner { max-width: 540px; margin: 0 auto; }
  .btn-calculate {
    width: 100%; background: linear-gradient(180deg, #0284c7 0%, #0369a1 100%);
    color: #fff; border: 1px solid rgba(255, 255, 255, 0.15); padding: 14px;
    border-radius: var(--radius-md); font-size: 0.92rem; font-weight: 700; cursor: pointer;
    box-shadow: 0 4px 20px rgba(2, 132, 199, 0.4); transition: var(--transition);
  }
  .btn-calculate:active { transform: scale(0.98); }
  .btn-calculate:disabled { opacity: 0.5; cursor: not-allowed; }

  #status { font-size: 0.78rem; color: var(--primary); text-align: center; margin-top: 6px; min-height: 1.2em; font-weight: 600; }
</style>
</head>
<body>
<div class="shell">
  ...
  <div class="panel">
    <div class="control-label">
      <span>Saved Sections Memory</span>
      <span class="hint">Department & Section</span>
    </div>
    <select id="sectionSelect" onchange="onSectionSelect(this.value)">
      <option value="">-- Choose Stored Department / Section --</option>
    </select>

    <div class="grid-2">
      <div>
        <div class="control-label">Department</div>
        <input type="text" id="deptInput" placeholder="e.g. ECE" value="ECE">
      </div>
      <div>
        <div class="control-label">Section</div>
        <input type="text" id="secInput" placeholder="e.g. Section C2" value="Section C2">
      </div>
    </div>
    ...
    <div class="dropzone" id="uploadBox">
      ...
      <div class="dropzone-state-loaded" id="uploadPreview">
        ...
        <div class="dz-meta">
          ...
          <div class="dz-tag">✓ Ready for extraction</div>
        </div>
      </div>
    </div>
    ...
    <div class="grid-2">
      <div>
        <div class="control-label">Batch Rule</div>
      ...
      <div class="disclosure-body">
        <div class="cal-matrix">
          <div class="cal-matrix-header">
            ...
            <div class="cal-matrix-title" id="calMonthTitle">September 2026</div>
...
<script>
  ...
  function saveYearBatch(val) { localStorage.setItem('my_year_batch', val); }
  ...
  async function loadSectionsList() {
    try {
      ...
      select.innerHTML = '<option value="">-- Choose Stored Department / Section --</option>';
      for (const secKey in serverSections) {
        const opt = document.createElement('option');
        opt.value = secKey;
        opt.innerText = secKey;
  ...
  function onSectionSelect(secKey) {
    if (serverSections[secKey]) {
      activeTimetable = serverSections[secKey].timetable || serverSections[secKey];
      document.getElementById('status').innerText = `✓ Loaded: ${secKey}`;
    }
  }
  ...
  async function calculateAll() {
    ...
    if (!activeTimetable && !selectedBase64) {
      alert("Please choose a stored section or upload a timetable screenshot.");
      return;
    }
  ...
  function runClientCalculation() {
    ...
    for (let d = new Date(start); d <= today; d.setDate(d.getDate() + 1)) {
      ...
      if (dayIndex === 6 && !isFresher && !isPlacement) continue;
  ...
  function renderResults(data, futureN) {
    ...
    radialMeter.style.strokeDashoffset = circumference - (fillValue / 100) * circumference;
